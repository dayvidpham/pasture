...
// --epoch it lists all recorded epochs; with --epoch it shows a full detail
// view for that epoch.
...
Without --epoch: lists all known epochs with their current phase and event count.

With --epoch: shows the full detail view for that epoch — current phase, agent
...
All output reads directly from the local pasture database — no running daemon
is contacted and no state is changed.`,
	Args: cobra.NoArgs,
...
func init() {
	statusCmd.Flags().String("epoch", "", "Epoch ID to inspect (omit to list all epochs)")
