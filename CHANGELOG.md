...
- `pastured` now chooses its exit code from the kind of failure instead of
  ...
  budget or a stop signal ends startup, 4 for a configuration problem, and 5
  for a storage or schema failure. Scripts and service units that treated any
  non-zero exit as the same fault need no change; those that test for exactly
  1 must be updated.
- `pastured` reports a stop that did not finish in time. The message names the
  ...
  that was cut off stays pending and is picked up on the next start.
