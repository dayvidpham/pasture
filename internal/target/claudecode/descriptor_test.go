...
func TestDescriptorPublishesPinnedClaudeIdentity(t *testing.T) {
	...
	wantContract := runtime.ClaudeCode2_1_261().ID()
	assert.Equal(t, wantContract, d.RuntimeContractID(),
		"the target must publish the exact contract identity it was compiled under")
	assert.Equal(t, "claude-code/claude-code@"+runtime.ClaudeCode2_1_261().Versions().Min().String(), d.RuntimeContractID().String(), "the public spelling is harness/harness@version, the version read from the runtime contract")
...
func TestNewTargetDescriptorRejectsNonClaudeContract(t *testing.T) {
	...
	openCodeContract := runtime.OpenCode1_18_29().ID()
