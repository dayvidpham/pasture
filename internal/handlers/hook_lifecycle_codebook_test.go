...
// TestHookLifecycleCodebookReadSurface exercises the production `hook lifecycle
...
// reported content digest == codebook.Active().Content).
func TestHookLifecycleCodebookReadSurface(t *testing.T) {
	t.Parallel()
	...
	code, err := handlers.HookLifecycleCodebook(ctx, &before, handlers.HookLifecycleCodebookInput{DBPath: dbPath}, "json")
	require.NoError(t, err)
	require.Zero(t, code)
	var beforeView codebookReadView
	...
	// Deliver a valid Claude SessionStart, which lazily activates the codebook.
	...
	_, err = handlers.HookLifecycleResponse(ctx, handlers.HookLifecycleInput{
		...
		Input: bytes.NewReader(raw), Clock: fixedLifecycleClock{}, Operations: fixedLifecycleOperations{id: "test.codebook.delivery"},
	})
	require.NoError(t, err)

	// After delivery: the coordinate is journaled, and the body is content-addressed.
	var after bytes.Buffer
	code, err = handlers.HookLifecycleCodebook(ctx, &after, handlers.HookLifecycleCodebookInput{DBPath: dbPath, Body: true}, "json")
	require.NoError(t, err)
	require.Zero(t, code)
	var afterView codebookReadView
