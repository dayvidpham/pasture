...
// registerQueuesAt builds an engine on dbPath so the queues are registered in
// that database, exactly as starting the daemon once would, and stops it again.
// The queue settings survive the engine, because they are rows in the database.
func registerQueuesAt(t *testing.T, dbPath string) {
	t.Helper()
...
func TestSetQueueConcurrency_IsReplacedWhenTheDaemonStartsAgain(t *testing.T) {
	...
	registerQueuesAt(t, dbPath) // the daemon starts once
	...
	registerQueuesAt(t, dbPath) // and starts again, with its own configured limit
	requireStoredConcurrency(t, dbPath, engine.SliceQueueName, engine.DefaultSliceQueueConcurrency)
}
