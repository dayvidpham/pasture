...
func TestDeriveObservationAndExplicitHumanResponse(t *testing.T) {
	...
	for _, event := range []runtime.ClaudeLifecycleEvent{
		...
		t.Run(event.NativeName(), func(t *testing.T) {
			t.Parallel()
			derivation, err := middleend.Derive(realL2(t, event), metamodel.Active())
			if err != nil {
...
func TestDeriveGateUsesCanonicalOrderAndProceed(t *testing.T) {
	t.Parallel()
	derivation, err := middleend.Derive(realL2(t, runtime.ClaudeEventPreToolUse), metamodel.Active())
	if err != nil {
		t.Fatalf("Derive() error = %v", err)
	}
...
func TestClaudeGateDerivationRegressionIsNonLive(t *testing.T) {
	...
	derivation, err := middleend.Derive(event, metamodel.Active())
	if err != nil {
		t.Fatalf("Derive() error = %v", err)
	}
	interpreted, err := receipt.NewInterpreted(event, event.Origin().Contract(), metamodel.Active())
	if err != nil {
