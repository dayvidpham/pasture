...
// governs behaviour. It is 20 rows, and it is NOT the same set as the rows the
...
//   - 2 rows differ only between two NON-BLOCKING arms: the Codex SessionStart
//     and SubagentStart observations read report-and-continue in the manifest
...
func TestTheManifestAndTheRuntimeDisagreeOnExactlyTheseRows(t *testing.T) {
	...
	if total != 20 {
		t.Fatalf("the recorded divergence covers %d rows, want 20: 11 Claude rows plus 9 Codex rows", total)
	}
}
