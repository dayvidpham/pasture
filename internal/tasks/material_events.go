...
const (
	...
	// FamilyEpochDecisionRecorded records a typed lifecycle/material reference to the
	// canonical decision fact and real Activity row produced by the same operation.
...
// EpochDecisionRecordedEvent records the lifecycle/material reference to a canonical
// decision fact. The event never carries decision detail: Facts().QueryDecisions is the
// sole authority for reopen-safe decision reconstruction.
...
func (e EpochDecisionRecordedEvent) validate() error {
	...
	if e.Activity == (provenance.ActivityID{}) || e.Decision == "" || e.Kind == "" {
		return invalidFieldErr("EpochDecisionRecordedEvent", "incomplete", "a non-empty activity, decision, and kind")
	}
