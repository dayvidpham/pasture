...
type RecordInput struct {
	...
	Trust                               Trust
	LastAction, LastOutcome, Diagnostic string
}
...
func (r Record) ArtifactID() string                { return r.value.ArtifactID() }
func (r Record) Version() string                   { return r.value.Version() }
func (r Record) Selector() string                  { return r.value.Selector() }
...
// Inventory is a global-only facade backed by a registry.Store.
type Inventory struct{ store registry.Store }

func New() Inventory                       { return Inventory{store: registry.New()} }
...
func (i Inventory) Marshal() ([]byte, error) { return i.store.Marshal() }
func Parse(data []byte) (Inventory, error) {
	s, err := registry.Parse(data)
	return Inventory{store: s}, err
}
func Load(path string) (Inventory, error) {
	s, err := registry.Load(path)
	return Inventory{store: s}, err
}
func Save(path string, i Inventory) error { return registry.Save(path, i.store) }
